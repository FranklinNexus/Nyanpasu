...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.stackContainer.setOnClickListener {
            ...
            if (isDualMode && homeFile.exists() && lockFile.exists()) {
                ...
                bounceAnimate(binding.cardFront)
            }
        }
    ...
    private fun loadPreview() {
        ...
        // 强制复位 Front Card 的属性 (防止动画卡在半路导致透明或消失)
        binding.cardFront.alpha = 1f
        binding.cardFront.scaleX = 1f
        binding.cardFront.scaleY = 1f
        binding.cardFront.translationY = 0f
        ...
        if (isDualMode) {
            ...
            // 复位 Back Card
            binding.cardBack.alpha = 0.5f
            binding.cardBack.scaleX = 0.9f
            binding.cardBack.scaleY = 0.9f
            binding.cardBack.translationY = 0f
            
            if (isPreviewingHome) {
                loadImageToView(homeFile, binding.ivFront) // 确保用的是 ivFront
                loadImageToView(lockFile, binding.ivBack)
                binding.tvViewIndicator.text = "Editing: Home Screen 🏠"
                binding.tvViewIndicator.setTextColor(Color.parseColor("#FF80AB"))
            } else {
                ...
                binding.tvViewIndicator.setTextColor(Color.parseColor("#64B5F6"))
            }
        } else {
            // === 单图模式 ===
            binding.cardBack.visibility = View.GONE
            binding.tvViewIndicator.visibility = View.INVISIBLE
    ...
    private fun loadImageToView(file: File?, imageView: android.widget.ImageView) {
        if (file != null && file.exists()) {
            // 使用 BitmapFactory 重新解码，确保刷新
            val bitmap = BitmapFactory.decodeFile(file.absolutePath)
            imageView.setImageBitmap(bitmap)
        } else {
            // 如果文件不存在（刚安装），可以显示个占位图或空
    ...
    private fun animateStackSwap() {
        ...
        back.animate()
            ...
            .withEndAction {
                ...
                loadPreview() // 重新加载数据
                // 此时数据已交换，属性已在 loadPreview 中复位，无需额外操作
            }
            .start()
    }
    ...
    private fun updateToggleButtons() {
        // Pink (#FF80AB), Blue (#64B5F6), Gray (#EEEEEE)
        ...
        loadPreview() // 按钮状态改变会影响预览（是否显示堆叠）
    }
