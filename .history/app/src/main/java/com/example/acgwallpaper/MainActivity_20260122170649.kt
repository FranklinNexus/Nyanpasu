...
class MainActivity : AppCompatActivity() {
    ...
    // --- 🖼️ 所见即所得壁纸应用 (WYSIWYG Apply with Crop) ---
    private fun applyCurrentToTarget(flag: Int, isHome: Boolean) {
        ...
        if (drawable == null) {
            // 回退到文件读取
            fallbackApplyFromFile(flag, isHome)
            return
        }

        lifecycleScope.launch(Dispatchers.IO) {
            try {
                // ✨ 获取 PhotoView 当前可见区域的 Bitmap（用户看到的部分）
                val visibleBitmap = getVisibleBitmap(photoView)
                
                if (visibleBitmap != null) {
                    val wm = WallpaperManager.getInstance(this@MainActivity)
                    wm.setBitmap(visibleBitmap, null, true, flag)
                    
                    withContext(Dispatchers.Main) {
                        val target = if (flag == WallpaperManager.FLAG_SYSTEM) "Home" else "Lock"
                        Toast.makeText(this@MainActivity, "$target wallpaper applied! (Cropped) ✨", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    // 裁剪失败，回退到文件
                    fallbackApplyFromFile(flag, isHome)
    ...
    // 回退方法：直接从文件读取（如果裁剪失败）
    private fun fallbackApplyFromFile(flag: Int, isHome: Boolean) {
        val homeFile = File(filesDir, "wallpaper_home.png")
        val lockFile = File(filesDir, "wallpaper_lock.png")
